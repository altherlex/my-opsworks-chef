...
module ProcessManager
  module Daemon
    class Master
      ...
      def self.status
        if (pid = find_pid) && ProcessManager::process_running?(pid)
          pid
        else
          # does not run
          clean_stale_pid
      ...
      def start
        ...
        # master does nothing apart from replacing dead children and forwarding signals and
        # it will abort if pid file information doesn't refer to the current process
        loop do
          current_pid = self.class.find_pid
          if $$ == current_pid
            sleep 1
          else
            puts "Aborting master with pid #{$$}, because doesn't match expected PID #{current_pid}"
            ProcessManager::Log.error("Aborting master with pid #{$$}, because doesn't match expected PID #{current_pid}")
            self.class.abort
          end
        end
      end
      ...
      def stop
        if (pid = self.class.find_pid)
          ...
          ProcessManager::Log.info("Nothing running that could be stopped")
        end
      end
      ...
      def self.find_pid
        found_pid = File.read(pid_file).chomp.to_i rescue nil
        # handle empty file so what we do not confuse the init process for the master process
        found_pid == 0 ? nil : found_pid
      end
