...
check_size () {
  if [[ -e "$(file_path "${PACKAGE_URL}" )" ]]; then
    ...
    if [[ "${_expected_size}" -eq "${_actual_size}" ]]; then
      ...
      local _ix_amz_id _x_amz_request_id
      _ix_amz_id="$(echo -n "${_http_headers}" | awk '/^x-amz-id-2/ {print $2}' | tr -d '\r')"
      _x_amz_request_id="$(echo -n "${_http_headers}" | awk '/^x-amz-request-id/ {print $2}' | tr -d '\r')"

      log "[ERROR] File size test failed. (ix-amz-id-2: ${_ix_amz_id} - x-amz-request-id: ${_x_amz_request_id}) url: ${PACKAGE_URL}"
      return 1
    fi
  else
...
while getopts "c:d:r:l:t:u:h:w" optname
do
  case "$optname" in
    "c")
